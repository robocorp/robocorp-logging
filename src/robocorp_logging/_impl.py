...
class _RobotOutputImpl:
    ...
    def close(self):
        ...
        if log_html:
            ...
            print(f"RFstream log (html): {target}")

            with open(target, "wb") as stream:
                from robocorp_logging import index

                index_html_contents = index.INDEX_HTML_CONTENTS

                # We want to replace the string so that we write
                sample_contents_index = index_html_contents.index("getSampleContents")
                return_json_parse_index = index_html_contents.index(
                    "return JSON.parse", sample_contents_index
                )

                end_of_json_parse_index = (
                    index_html_contents.index(")}}", return_json_parse_index) + 1
                )

                stream.write(
                    index_html_contents[:return_json_parse_index].encode("utf-8")
                )
...
"""
                ...
                stream.write(
                    index_html_contents[end_of_json_parse_index:].encode("utf-8")
                )
